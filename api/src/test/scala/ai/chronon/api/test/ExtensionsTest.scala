...
class ExtensionsTest {
  ...
  @Test
  def externalOfflineGroupByBackfill(): Unit = {
    val metadata = Builders.MetaData(name = "test", team = "team", description = "metadata description")
    val groupBy = Builders.GroupBy(
      sources = Seq(Builders.Source.events(query = null, table = "db.gb_table", topic = "test.gb_topic")),
      keyColumns = Seq("a", "c"),
      metaData = metadata)
    ...
    val join1 = Builders.Join(
      left = Builders.Source.events(query = null, table = "db.join_table", topic = "test.join_topic"),
      externalParts = Seq(Builders.ExternalPart(externalSource = externalSource, prefix = "ext"))
    )
    assertEquals(join1.getRegularAndExternalJoinParts.map(part => part.fullPrefix).head, "ext_test")
